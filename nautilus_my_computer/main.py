...
class MyComputerExtension(GObject.GObject, Nautilus.MenuProvider):
    ...
    def _find_sidebar_listbox(self, nautilus_sidebar) -> Gtk.ListBox | None:
        ...
        for w in _all_widgets(search_root):
            if isinstance(w, Gtk.ListBox) and w.has_css_class("navigation-sidebar") and not _is_ours(w):
                return w

        for w in _all_widgets(search_root):
            if isinstance(w, Gtk.ListBox) and w.has_css_class("places-sidebar-list") and not _is_ours(w):
                return w
